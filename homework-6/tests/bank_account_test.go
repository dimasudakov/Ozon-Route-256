...
func NewBankAccountServiceFixture() *BankAccountServiceFixture {
	...
	bankAccountController := account.NewBankAccountController(bankAccountService, kafkaLogsTestTopic.Logger)
...
func TestCreateBankAccount(t *testing.T) {
	...
	var (
		...
		validBankAccountDtoJSON, _   = json.Marshal(validBankAccountDto)
		invalidBankAccountDtoJSON, _ = json.Marshal(invalidBankAccountDto)
	)
	...
	tests := []struct {
		...
		expectedLogs    []logging.LogMessage
		idExists        bool
	}{
		{
			...
			expectedLogs: []logging.LogMessage{
				{
					LogLevel:    "INFO",
					RequestType: "POST",
					Body:        string(validBankAccountDtoJSON),
				},
			},
			idExists: true,
		},
			...
			expectedLogs: []logging.LogMessage{
				{
					LogLevel:    "INFO",
					RequestType: "POST",
					Body:        string(invalidBankAccountDtoJSON),
				},
				{
					LogLevel:    "ERROR",
					RequestType: "POST",
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			db.SetUp(t)
			kafkaLogsTestTopic.SetUp(t, len(tc.expectedLogs))
			defer func() {
			...
			assert.True(t, kafkaLogsTestTopic.CheckLogs(t, tc.expectedLogs), "Logs are not correct")
		})
	}
}
...
func TestUpdateBankAccount(t *testing.T) {
	...
	var (
		...
		validUpdatedAccountDtoJSON, _   = json.Marshal(validUpdatedAccountDto)
		notExistingAccountId            = uuid.New()
		invalidUpdatedAccountDto        = fixtures.NewBankAccountDtoBuilder().Invalid().Balance(5000).Build()
		invalidUpdatedAccountDtoJSON, _ = json.Marshal(invalidUpdatedAccountDto)
	)
			...
			expectedLogs: []logging.LogMessage{
				{
					LogLevel:    "INFO",
					RequestType: "PUT",
					Body:        string(validUpdatedAccountDtoJSON),
				},
			},
			idExists: true,
		},
			...
			expectedLogs: []logging.LogMessage{
				{
					LogLevel:    "INFO",
					RequestType: "PUT",
					Body:        string(invalidUpdatedAccountDtoJSON),
				},
				{
					LogLevel:    "ERROR",
					RequestType: "PUT",
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.True(t, kafkaLogsTestTopic.CheckLogs(t, tc.expectedLogs))
		})
	}
}
